...
$(function () {
    ...
    if (dtUserTable.length) {
        oTable = dtUserTable.DataTable({
            "ajax":{
                url:assetPath + '/visitsLastStored',
            } ,
            "columns": [
                {
                    orderable: false,
            ...
            initComplete: function () {
                ...
                this.api()
                    ...
                    .every(function () {
                        var column = this;
                        var select = $(
                            '<select id="UserRole" class="form-control text-capitalize mb-md-0 mb-2"><option value=""> تاريخ الحركة </option></select>'
                        )
                            .appendTo('.visit_time_list')
                            .on('change', function () {
                                var val = $.fn.dataTable.util.escapeRegex($(this).val());
                                column.search(val ? '^' + val + '$' : '', true, false).draw();
                            });
    ...
    // $(document).on('keypress', '#car_id', function (e) {
    //     $('#add-form').submit();
    // });
    ...
    $('#add-form').submit(function(e){
        ...
        $.ajax({
            ...
            .done(function(response) {
                    ...
                    $('#car_id').val('');
                    $('#car_id').focus();
                window.open(assetPath+'/visit/print/'+response.items.id, "_blank");
            })
            .fail(function(data) {
                $('#visitor_search_Error').text(data.responseJSON.message);
                $('#car_id').val('');
                $('#car_id').focus();
